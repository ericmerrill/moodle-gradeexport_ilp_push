...
/**
 * Event for when there is an ILP error while submitting a grade.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class grade_sent_error extends \core\event\base {
    ...
    public static function create_from_saved_grade(saved_grade $grade) {
        $resubmit = false;
        if ($grade->status == saved_grade::GRADING_STATUS_RESUBMIT) {
            $resubmit = true;
        }
    ...
    public static function get_name() {
        return get_string('event_grade_sent_error', 'gradeexport_ilp_push');
    }

    public function get_description() {
        $courseilpid = $this->other['courseilpid'];
        if (!empty($this->other['resubmitted'])) {
            $resubmit = " Marked for submission to Banner.";
        } else {
            $resubmit = " Only saved locally, not marked for submission to Banner.";
        }
        ...
        return "Banner returned an error when submiting the grade id '$this->objectid' for the user with ".
                "id '$this->relateduserid' for ILP course '$courseilpid'.".$message.$resubmit;
    }
}
